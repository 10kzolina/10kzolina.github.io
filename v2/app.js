(function () {
  ...
  function renderEventTopbar() {
    ...
    const formatShortIsoDate = (isoDate) => {
    ...
    const buildMarqueeGroup = (isClone = false) => `
      <div class="event-topbar-group"${isClone ? ' aria-hidden="true"' : ""}>
        <div class="event-topbar-status"${isClone ? "" : ' aria-live="polite"'}>
        ...
        <a class="event-topbar-item event-topbar-date"
          ...
          type="text/calendar"
          aria-label="Añadir la 10K Zolina al calendario"${isClone ? ' tabindex="-1"' : ""}>
    ...
    topbar.innerHTML = `
      <div class="event-topbar-inner">
        <div class="event-topbar-marquee" aria-label="Información de la carrera">
          <div class="event-topbar-track">
            ${buildMarqueeGroup(false)}
            ${buildMarqueeGroup(true)}
          </div>
        </div>
